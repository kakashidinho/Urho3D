...
//    panelSurface->SetViewport(0, panelViewport);
    ...
    for (bool small_ : {true, false}) {
        ...
        switch(colorSet_) {
        case 1: {
            panelPos = small_ ? Vector3(-0.83787f, 0.4306f, 2.01268f)
                             : Vector3(-4.49769f, 0.0f, 2.59509f);

            panelRot = small_ ? Quaternion(-120.0f, Vector3::UP)
        ...
        case 3: {
            panelPos = small_ ? Vector3(-3.46253f, 0.4306f, -0.47611f)
        ...
        Node* panelNode{ node_->CreateChild(small_ ? "SmallPanel" : "BigPanel") };
        panelNode->SetPosition(panelPos);
        panelNode->SetRotation(panelRot);
        panelNode->SetScale(small_ ? 1.0f : 3.472769409f);
        panelNode->SetEnabled(small_);
