...
level::level(std::string level_filename)
{
	...
	String level_full_path = globals::instance()->context->GetSubsystem<FileSystem>()->GetProgramDir()+"Data/Urho3DPlatformer/"+ level_filename.c_str();
    pugi::xml_parse_result result=doc.load_file(level_full_path.CString());
    if(!result)
    {
